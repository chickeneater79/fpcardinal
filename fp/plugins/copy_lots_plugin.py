...
def init_commands(cardinal: Cardinal):
    ...
    def get_lots_info(tg_msg: Message, profile: FunPayAPI.types.UserProfile) -> list[FunPayAPI.types.LotFields]:
        ...
        for i in profile.get_lots():
            ...
            while attempts:
                try:
                    result.append(cardinal.account.get_lot_fields(i.id, i.subcategory.id))
    ...
    def act_copy_lots(m: Message):
        ...
        tg.set_user_state(m.chat.id, result.id, m.from_user.id, CBT_COPY_LOTS)
    ...
    def act_create_lots(m: Message):
        ...
        result = bot.send_message(m.chat.id,
                                  ...
                                  reply_markup=skb.CLEAR_STATE_BTN)
        tg.set_user_state(m.chat.id, result.id, m.from_user.id, CBT_CREATE_LOTS)
    ...
    def create_lots(m: Message):
        ...
        try:
            ...
            for i in data:
                try:
                    time.sleep(1)
                    lot = FunPayAPI.types.LotFields(0, int(i["node_id"]), i)
            ...
            logger.debug("TRACEBACK", exc_info=True)
            bot.send_message("❌ Не удалось создать лоты.")
            return
